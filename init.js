...
function init(state = 0) {
  ...
  canvas.width = luckyBag
    ? (Math.max.apply(null, CategoryNUM) + 1) * (CELL_SIZE + col_padding) + caculateField
    : (Math.max.apply(null, CategoryNUM) + 1) * (CELL_SIZE + col_padding);
  // add width
  var wid = 700;
  ...
  luckyBag
    ? context.fillRect(0, 0, canvas.width + caculateField, canvas.height)
...
function fillRect(x, y, color) {
  ...
  context.fillRect(
    (x + 1) * (CELL_SIZE + col_padding) + marginLeft,
    y * (CELL_SIZE + row_padding) + marginTop,
    CELL_SIZE,
    CELL_SIZE
  );
